...
| FOUNDATION-P0-16 | `lib/ai/` — shared, read-only, advisory-only LLM summarization primitive | Partial — 2026-09-16: interface/contract built and unit-tested (`lib/ai/summarize.ts`, `lib/shared/types/ai.ts`) — no DB client import (boundary enforced by the file's own shape, not just documented), `summarize()` deterministically throws `AiNotConfiguredError` until a provider is wired; the actual provider/credential decision remains explicitly open (same question as `PLATFORM-P0-05.2`), so no real LLM call exists yet, per the story's own "stub/interface can be built without one" allowance — see audit log |
