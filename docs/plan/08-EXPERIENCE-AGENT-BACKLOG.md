...
### EXPERIENCE-P0-16 — Light-console screens 4–12 and the MCP boards

Restyle the remaining mockup screens onto the shared primitives with real data only: Access graph, Policies, Runtime Gateway, Risk & Investigations, Audit & Compliance, Integrations catalog, Administration, Investigation detail, Authorization request, MCP servers/detail/tools/mapping. Screens backed by a not-yet-built capability wait for that story rather than showing fabricated data.

### EXPERIENCE-P0-17 — Access wording: friendly label with the technical term

User decision: screens say "Approved (SHOULD)", "Effective Access (CAN)", "Observed (DID)" and "Current Request (NOW)" — both terms together, everywhere the three views appear. `CLAUDE.md` §9 and UI rules §11 are unchanged (SHOULD/CAN/DID stay the canonical model terms).
