...
| EXPERIENCE-P0-04 | Action Safety (confirmation, scope preview, bulk-action reporting) | Partial — `ConfirmActionDialog` now wired to five real destructive/consequential actions across four modules (Risk's remediate, Identity's duplicate-merge, Compliance's revoke decision, Foundation's role removal); bulk-action reporting still has no real bulk endpoint to prove itself against, and a handful of narrower destructive actions (e.g. direct access-grant revocation, SSO connection deletion) remain unconfirmed — see audit log |
...
| EXPERIENCE-P0-08 | Data Table Primitive (sort/filter/pagination/saved URL state/responsive card transform) | Partial — `DataTable`/`useTableState`, plus `SimpleDataTable`, now consumed by six real screens (Agent Inventory, Access Applications, Policies, the Risk index's agent/finding-count table, Integrations); pagination/sort/filter still run client-side over the already-fetched full list since the underlying `list*()` contracts have no server-side pagination parameters yet (a documented stopgap, not the primitive's own limitation); Access Requests, Compliance Items, Audit and a few smaller lists still use the plain `Table` primitive — reasonable for their current size, but not a literal "every list screen" reading of the story |
