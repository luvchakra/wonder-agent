...
| EXPERIENCE-P0-03 | Domain Screens (Agent/Access/Runtime/Rogue/Certification/etc.) | Partial — the four per-agent worked layouts (Agent Detail/Access/Runtime/Risk, the PRD's §35/§36) are now built as one coherent tabbed Agent Detail experience (new `AgentTabs` primitive + all four pages restyled onto Card/Badge/Table); Access/Compliance/Integration/Foundation's other list/settings screens and the standalone Rogue Agent Detail layout remain bare — see audit log |
