...
import { setRoleStatus } from "@/lib/rbac/customRoles";

// FOUNDATION-P0-25 — POST { status: active | inactive } for a custom role.
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await setRoleStatus(ctx.tenantId!, ctx.userId, (await params).id, body.status);
